...
def parse_args():
    ap = argparse.ArgumentParser("Pipeline inference: clf -> (det + seg)")
...
def main():
    ...
    # detector
    det_model, det_device, det_size = load_detector(args.det_config, args.det_ckpt, device)
    ...
    with torch.no_grad():
        ...
        det_proc = postprocess(det_out, target_sizes=torch.tensor([[orig_h, orig_w]], device=det_device))[0]
    print("Detections:")
